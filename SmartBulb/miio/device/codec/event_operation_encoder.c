	...
	for(int i = 0;i < opt->arguments->size;i++) {
		...
		switch(opt->arguments->arguments[i].value->format) {
			...
			case PROPERTY_FORMAT_STRING : {
				str_n_cat(out, 2, opt->arguments->arguments[i].value->data.string.value, " ");
			}
			break;

			case PROPERTY_FORMAT_NUMBER : {
				if(opt->arguments->arguments[i].value->data.number.type == DATA_NUMBER_INTEGER) {
					char integer_buf[VALUE_MAX_LEN] = {0};
					snprintf(integer_buf, VALUE_MAX_LEN, "%d", (int)opt->arguments->arguments[i].value->data.number.value.integerValue);
					str_n_cat(out, 2, integer_buf, " ");
				} else if (opt->arguments->arguments[i].value->data.number.type == DATA_NUMBER_FLOAT) {
					char float_buf[VALUE_MAX_LEN] = {0};
					snprintf(float_buf, VALUE_MAX_LEN, "%f", opt->arguments->arguments[i].value->data.number.value.floatValue);
